...
{
	/*
	   The POS board does NOT have an eeprom. We don't need this code to
	   run on multiple configurations, so instead of probing the I2C bus,
	   we'll just manually set values here (for now copying the configs
	   for am335 evm kit.
	*/
	header->magic = 0xEE3355AA;
	strcpy(header->name, "A335X_SK");
	strcpy(header->version, "1.5");
	strcpy(header->config, "SKU#02");
